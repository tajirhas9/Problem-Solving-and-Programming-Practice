...
ll st[MAX*4+10],lazy[MAX*4+10],a[MAX+10];

void update_node(ll node, ll val) {
	st[node] += val;
	lazy[node] += val;
}
...
void update(ll node,ll left,ll right, ll l, ll r, ll val) {
	...
	ll mid = (left+right)>>1;
	if(r<= mid) 
		update(lc, left, mid, l, r, val);
	else if(l>mid)
		update(rc,mid+1,right,l,r,val);
	else {
		update(lc,left,mid,l,r,val);
		update(rc,mid+1,right,l,r,val);
	}
...
ll query(ll node, ll left, ll right, ll l, ll r) {
	...
	ll mid = (left+right)>>1;
	if(r<=mid)
		return query(lc,left,mid,l,r);
	else if(l> mid) 
		return query(rc, mid+1, right, l, r);
	else
		return min( query(lc,left,mid,l,r) , query(rc,mid+1,right,l,r) );
}
...
int main()
{
	...
	while(q--) {
		...
		//	cout << q << " " << l << " " << r << endl;
			if(l>r) {
				ll p1 = query(1,0,n-1,l,n-1);
				ll p2 = query(1,0,n-1,0,r);
				printf("%I64d\n",min(p1,p2));
			}
			else 
				printf("%I64d\n",query(1,0,n-1,l,r));
