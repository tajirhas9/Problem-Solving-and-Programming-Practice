...
ll query(ll node, ll left, ll right, ll l) {
	if(left==right)
		return st[node];
	push(node,left,right);
	...
	if(l > tm)	return query(rc, tm+1, right, l);
	else
		return query(lc, left, tm, l);;
}
...
int main()
{
	...
	for(int i = 1; i <= m; ++i) {
		ll val = p[i].S * op[i];
		dbg(i , p[i].F , p[i].S , op[i] , val);

		update(1,0,n-1,p[i].F.F , p[i].F.S , val);
	}
