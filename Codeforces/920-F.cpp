...
void seive(int limit) {
	...
	for(int i = 1; i< limit; i ++ ) {
		...
		for(int j = 0; prime[j] <= sqrtn; j++ ) {
			if( ( prime[j] == i/prime[j] ) && ( i % prime[j] == 0 ) ) 	cnt ++ ;
			else if(i % prime[j] == 0)	cnt += 2;
		}
		divisor.pb(cnt);
		cnt = 0;
	}
...
void build(ll node, ll left, ll right) {
	...
	st[node].F = st[2*node].F + st[2*node+1].F;
	st[node].S = 0;
}

void update(ll node, ll left, ll right, ll l, ll r) {
	if(left==right) {
		st[node].F = divisor[st[node].F];
		return;
	}
	...
	ll tm  = (left+right)>>1;
	if(r<=tm)	update(2*node, left, tm, l, r);
	else if(l>tm)	update(2*node+1, tm+1, right, l, r);
	else {
