...
int st[6*MAX+10][3], lazy[6*MAX+10];

void update_node(ll node, ll val,ll left,ll right) {
	for(int i = 1; i <= val; ++i) {
		ll x,y,z;
		x = st[node][0] , y = st[node][1] , z = st[node][2];
...
int query(ll node, ll left, ll right, ll l, ll r) {
	if(l <= left && r >= right)
		return st[node][0];
	push(node,left,right);
