...
<body>												<!-- Header -->
	<header>
		...
		<div style= "float : right;margin-top : -4.5%;clear : both;"><a href = '<?php echo "Dashboard.php?name=$name&id=$id&usrname=$usrname";?>'><?php echo $name;?></a></div>
	</header>
															<!--Header Ends -->
																<!-- Clock -->
	<time>
	...
	<div id = "contest_content" style = "padding : 13%;">
			<?php
				...
				$sql = "SELECT ID,Username,Score FROM users ORDER BY Score DESC;";
				$query =  mysqli_query($conn,$sql);
				echo "<table>";
				echo 	"<tr>";
				echo 		"<th>Username</th>";
				...
				while($info = mysqli_fetch_array($query))
				{
					...
					$usrid = $info['ID'];
					echo "<tr>";
					echo 	"<td><a href = 'profile.php?user=$user&id=$usrid'>".$user."</a></td>";
					echo 	"<td>".$score."</td>";
				}
				echo "</table>";
			?>
