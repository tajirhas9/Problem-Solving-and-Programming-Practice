...
<body>												<!-- Header -->
	...
	<time>
		<clock id = "qwe">
			<script>
				var now = new Date(<?php echo time("h:i:s") * 1000 ?>);
	...
	<div id = "contest_content" style = "padding : 13%;">
		<?php
			...
			$sql = "SELECT serial_number,problemNumber,problemName,contestStart,contestDuration FROM contestProblem WHERE contestName = '".$contestName."';";
			$query =  mysqli_query($conn,$sql);
			echo "<table class = 'problemTable' border='1'>
					<tr>
						<th>No.</th>
						<th>Problem</th>
					</tr>";
			while($info = mysqli_fetch_array($query))
			{
				$problemSerial = $info['serial_number'];
				$contestStart = $info["contestStart"];
				$contestDuration = $info["contestDuration"];
				echo "<tr>";
				echo "<td align = 'center'><a href = 'problemOnContest.php?problemSerial=$problemSerial' class = 'contestList'>".$info['problemNumber']."</a></td>";
				echo "<td><a href = 'problemOnContest.php?problemSerial=$problemSerial' class  = 'contestList'>".$info['problemName']."</a></td>";
				echo "</tr>";
			}
			echo "</table>";
		?>
				<div id = "countdown" style = "float : right;clear : both;border : 2 px black;margin-right : 70px;"><p>here</p>
					<?php 
						...
						if(strtotime($currentDate) >= strtotime($contestStart))
						{
							...
							$fromTime = strtotime(date("Y-m-d h:i:s A"));
							$toTime = $endTime;
