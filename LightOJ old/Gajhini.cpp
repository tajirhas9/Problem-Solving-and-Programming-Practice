...
void build(ll node,ll left,ll right)
{
	...
	st[node].F = max(st[2*node].F,st[2*node+1].F);	
	st[node].S = min(st[2*node].S,st[2*node+1].S);
}
...
int main()
{
	...
	rep(kase, 1, T) {
		...
		//dbg(st,16);
		for(i=0; i+d-1 <n; i++) {
			pll tm = query(1,0,n-1,i,i+d-1);
			ans = max((tm.F-tm.S),ans);
			//cout << i << " " << i+d-1 << " " << (tm.F-tm.S) << endl; 
		} 
		printf("Case %lld: %lld\n",kase,ans);
		ans = -1;
	}
