...
void update_node(ll node, ll val,ll left,ll right) {
	st[node] = modAdd(st[node] , (val*(right-left+1)));
	lazy[node] = modAdd(lazy[node] , val);
}
...
ll query(ll node, ll left, ll right, ll l, ll r) {
	...
	if(r<= tm)	return query(lc, left, tm, l, r);
		...
		return modAdd( query(lc, left, tm, l , r) , query(rc, tm+1, right, l, r) );
}

ll getSum(ll l, ll r, ll k) {
	dbg(l,r,k);
	ll ratio = r - l + 2;
	ll a1 = query(1,0,n-1,l-1,r-1);
	dbg(a1,ratio);
	ll sum = modMul(a1 , modSub(1,modPow(ratio , k)));
	sum = modDiv(sum , modSub(1,ratio));
...
int main()
{
	...
	while(q--) {
		...
		ll to_add = getSum(l,r,k);
		dbg(to_add);

		update(1,0,n-1,l-1,r-1,to_add);
	}
