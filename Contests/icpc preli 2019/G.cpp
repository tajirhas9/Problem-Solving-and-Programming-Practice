...
public:
	...
	vector < long long > cumPhi;
	vector < long long > coSum;
	int N;
	...
	long long sumOfNonCoPrimesOfAnInteger(long long n) {
		...
		*/
		...
		ll sum = n*(n-1) / 2;
		sum -= sumOfCoPrimesOfAnInteger(n);
...
EulerTotient totient(10000007);
ll n,p;

bool isOk(ll x) {
	ll f = totient.cumPhi[n/x]+1;
