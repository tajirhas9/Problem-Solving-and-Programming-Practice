...
ll fx[] = {1,1,-1,-1};
ll fy[] = {1,-1,1,-1};
ll solve(ll pos,ll n,ll leftSum,ll rightSum,bool lft);
int main()
{
	optimize();
	ll i,j,k=0,n;
...
ll solve(ll pos,ll n,ll leftSum,ll rightSum,bool lft)
{
	...
	if(lft)
	{
		ll i;
		rep(i,0,9)
			p2 += solve(pos+1,n,leftSum+i,rightSum,lft);
	}
	else
	{
		ll i;
		rep(i,0,9)
		{
			if(rightSum+i<=leftSum)
				p1 += solve(pos+1,n,leftSum,rightSum+i,lft);
		}
	}
	return p1+p2;
}
