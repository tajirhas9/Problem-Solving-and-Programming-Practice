...
//ll fy[] = {1,-1,1,-1};
ll n,k;
unsigned long long dp[1805][10];
unsigned long long solve(int curLen,int cur,int prev);
int main()
{
	optimize();
	int i;
...
unsigned long long solve(int curLen,int cur,int prev)
{
	...
	if(prev==cur && cur==0)	return 0;
	if(curLen==n)	return 1;
	if(dp[curLen][cur])	return dp[curLen][cur];
	rep(i,0,k-1)
	{
		if(cur==0 && i==0)	continue;	
		p1 += solve(curLen+1,i,cur);
	}
	return (dp[curLen][cur] = p1);
}
