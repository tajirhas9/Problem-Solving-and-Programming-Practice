...
ll fy[] = {1,-1,1,-1};
ll dp[1000][1000];
ll solve(ll in_hand,ll dif,ll stairs);
int main()
{
	optimize();
	ll i,j,k,n;
...
{
	ll p1=0,i,prev=0;
	if(in_hand==0 && stairs>1)	return 1;
	if(dp[in_hand][dif])	return dp[in_hand][dif];
	rep(i,1,in_hand)
	{
		if((in_hand-(dif+i))==0 || (in_hand-(dif+i))>(dif+i))
		{
			p1 += solve(in_hand-(dif+i),dif+i,stairs+1);
		}
	}
	return (dp[in_hand][dif]=p1);
}
